...
def main_worker(gpu, ngpus_per_node, config, logger, model_dir):
  ...
  if config.dataset == 'cifar10' or config.dataset == 'cifar100':
    ...
    block = getattr(resnet_places, 'Bottleneck')(config.feat_size,
                                                 config.bottle_size,
                                                 groups=1,
                                                 base_width=64,
                                                 dilation=1,
    ...
    classifier1 = getattr(resnet_places, 'Classifier')(feat_in=config.feat_size,
                                                       num_classes=config.num_classes)
    classifier2 = getattr(resnet_places, 'Classifier')(feat_in=config.feat_size,
                                                       num_classes=config.num_classes)
  ...
  if config.dataset == 'cifar10':
    dataset = CIFAR10_LT(config.distributed,
                         root=config.data_path,
                         imb_factor=config.imb_factor,
                         batch_size=config.batch_size,
                         num_works=config.workers,
                         autoaug=config.autoaug,
                         randaug=randaug)

  elif config.dataset == 'cifar100':
    dataset = CIFAR100_LT(config.distributed,
                          root=config.data_path,
                          imb_factor=config.imb_factor,
                          batch_size=config.batch_size,
                          num_works=config.workers,
    ...
    dataset = ImageNet_LT(config.distributed,
                          root=config.data_path,
                          batch_size=config.batch_size,
                          num_works=config.workers,
                          randaug=randaug)
    ...
    dataset = Places_LT(config.distributed,
                        root=config.data_path,
                        batch_size=config.batch_size,
                        num_works=config.workers,
                        randaug=randaug)
  ...
  for epoch in range(config.num_epochs):
    ...
    if config.loss_type == 'CE':
      ...
      criterion = {
        "lc": LogitAdjust(cls_num_list=cls_num_list).cuda(config.gpu),
        "bcl": BalSCL(cls_num_list=cls_num_list).cuda(config.gpu)
      }
    elif config.loss_type == 'JSD':
      criterion = JSDDistillLoss(cls_num_list=cls_num_list).cuda(config.gpu)
    else:
    ...
    train(train_loader, balance_loader, model, classifier1, classifier2, criterion, optimizer,
          scaler, epoch, config, logger, block, head)
...
def train(train_loader,
  ...
  for i, (input1, target1) in enumerate(train_loader):
    ...
    with autocast(enabled=config.amp):
      if config.csa and back_images.shape[0] >= input1.shape[
          0] and epoch >= config.start_aug:                                                    # and epoch < config.num_epochs - config.end_aug:
        ...
        # save inputs 2 for self-distillation
        input2_aug = lam*aug_masks*aug_images + input2 * (1. - lam*aug_masks)
        # compute output
        if config.dataset == 'places':
          with torch.no_grad():
            feat2_a = model(input2_aug)
          feat2 = block(feat2_a.detach())
          output2_aug = classifier2(feat2)
        else:
          feat2 = model(input2_aug)
          output2_aug = classifier2(feat2)
        loss2 = F.cross_entropy(output2_aug, target2)
      else:
        loss2 = 0
        # with torch.no_grad():
        #     feat2 = model(input2)
      ...
      elif config.loss_type == 'JSD' and epoch < config.num_epochs - config.end_aug:
        if config.dataset == 'places':
          with torch.no_grad():
          ...
          feat2 = model(input2)
          output2 = classifier2(feat2)
        loss = criterion(output2, output2_aug, target2, epoch, config.num_epochs)
...
def validate(val_loader, model, classifier1, classifier2, criterion, config, logger, block=None):
  batch_time = AverageMeter('Time', ':6.3f')
  acc_meter = {'classifier1': AccMeter(), 'classifier2': AccMeter(), 'ensemble': AccMeter()}
  progress = ProgressMeter(
    len(val_loader), [batch_time, acc_meter['classifier1'].top1, acc_meter['classifier1'].top5],
    prefix='Eval: ')
  ...
  with torch.no_grad():
    ...
    for name in acc_meter.keys():
      ...
      logger.info((
        '* ({name})  Acc@1 {acc1:.3f}  HAcc {head_acc:.3f}  MAcc {med_acc:.3f}  TAcc {tail_acc:.3f}  '
        '(Best Acc@1 {best_acc1:.3f}).').format(name=name,
                                                acc1=acc1,
                                                acc5=acc5,
                                                head_acc=head_acc,
                                                med_acc=med_acc,
